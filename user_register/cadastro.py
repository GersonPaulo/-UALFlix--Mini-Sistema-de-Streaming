...
from editor import  EditorTxt, EditorBanner
...
class Cadastro :
    ...
    def add_mail(self):
        ...
        while True:
            ...
            if k == True:
                if  ('@' not in temp_var):
                    a = '=== E-mail invalido! ==='
                    edtxt.negrito_red('=== E-mail invalido! ===')
                if (temp_var in db_email):
                    edtxt.negrito_bright_white('E-mail already in database')
                    edtxt.negrito_red('=== E-mail invalido! ===')
    ...
    def update_user_email(self):
        try:
            temp_var = int(input('digite o user_id ----->'))
        ...
        for user in all_data:
            if temp_var == user["user_id"]:
                a = self.add_mail()
                update_mail = db_manager.update_user_email( a, user["user_id"])
